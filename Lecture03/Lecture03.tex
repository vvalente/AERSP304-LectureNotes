...
\begin{frame}[t]{Example: Tug-of-War on a Rotating Platform}
    In a game of Tug-of-War, participants are constrained to move only along a narrow platform. Moreover, the platform is rotating at angular velocity $\dot\theta\,\hat{\mathbf n}_3$. They move back and forth so that the distance from the center of the platform a point $P$ (point mass) is given by
    \[
    ...
    \begin{tikzpicture}[scale=0.8, line cap=round, line join=round]
        ...
        % r arrow
        \draw[->, thick, blue] (0.1,0.5) -- (1.45,0.5) node[midway, above] {$r$};
